 ...
 * \brief Transformations appliquées aux lettres
 * \author Marc Blanc-Patin et Bernier Paul
 * \version 1.0.0
 * \date 23 mai 2011
 *
 * Set de fonctions qui permettent de transformer la géométrie d'une lettre
 *
 */
...
Lettre transformation(Lettre lettre, int i, int width, int height,int z,int marge,float e, float offset)
{
	...
	// Déformation
	if (cfg_deform_sin) lettre = deformation_sin(lettre,'h');
